...
| Perf | Download timing | Medium | Partially Done | Safe `[Perf][Download]` / `[Perf][Analyze]` / `[Perf][MobileDownload]` timers. Share split stages + Analyze sub-stages (`analyze_ytdlp_metadata`, `analyze_qualities`, `analyze_total`, mobile `analyze_http`). First TikTok download sample interpreted; Analyze ~3s still needs sub-stage benchmark before cache/lighter metadata. No optimizations shipped. | 2026-07-26 | Measure before optimize |
