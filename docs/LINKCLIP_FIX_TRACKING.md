...
| Perf | Download / Analyze timing | Medium (Analyze open) | Partially Done | **TikTok-ready Done (production-verified):** AVC/H.264-preferring selector → `strategy=audio_only`, ffmpeg ~1.0–1.2s, `backend_total` ~3.8–4.1s (was ~13.4s / ~16.6s `full_transcode`). **`best` regression passed** (`strategy=skipped`, unchanged). FFmpeg normalize + download file streaming **log hygiene verified** (no path/storage-key leaks). **Open:** Analyze metadata cost (`--dump-json` ~3s). Share/Save not bottlenecks. See `docs/LINKCLIP_DOWNLOAD_PERFORMANCE.md`. | 2026-07-26 | Next: Analyze |
