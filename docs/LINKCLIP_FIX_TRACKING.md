...
| Perf | Download / Analyze timing | Medium | Partially Done | **TikTok-ready Done (production-verified).** **Analyze:** Redis 60s result cache (`analyze:result:v1:{urlHash}`) + in-flight dedupe shipped — repeat opens within TTL skip yt-dlp/Link upsert; quota still counts every POST; failures not cached. First-hit `--dump-json` still ~3s. Log hygiene verified. See `docs/LINKCLIP_DOWNLOAD_PERFORMANCE.md`. | 2026-07-26 | Measure Analyze cache in prod |
