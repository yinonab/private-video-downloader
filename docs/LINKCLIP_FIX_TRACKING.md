...
| Perf | Download / Analyze timing | High (TikTok-ready) | Partially Done | `tiktok_ready` now prefers AVC/H.264 (+ AAC) then falls back to former `best` selector; ffprobe/normalize unchanged. Re-benchmark pending to confirm remux/audio_only vs prior ~13.4s full_transcode. Analyze/Share/Save backlog unchanged. See `docs/LINKCLIP_DOWNLOAD_PERFORMANCE.md`. | 2026-07-26 | Measure after deploy |
