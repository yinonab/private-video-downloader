...
- **`EditVideoScreen`** — **scrollable** tool strip (**overflow arrows + soft edge fades**, RTL-aware) + single visible panel: **Trim**, **Speed**, **Format** (shape + Fill/Fit-blur + **rotation** — not a separate tab), **Captions** (**Auto captions** + **V2.4A draft**: **Generate draft** / **Regenerate draft** (confirmation before replacing cue text edits) on **`trim`**/**`speed`** timeline via **`POST /edits/captions/draft`**, **segment text edits in a keyboard-aware bottom sheet** (read-only preview rows in the list; Save/Cancel/Clear; empty text allowed locally), timing/source invalidation helpers + **V2.3 presets** + manual sizing / positioning / offsets; default control values match **Minimal** preset; **`EditCaptionsPreviewOverlay`** when Captions tab + Auto on), **Audio** (mute), **Quality** (compress preset); compose → **`POST /edits`** (**`captions.mode auto`** without draft vs **`segments`** when draft loaded) → poll → download.
