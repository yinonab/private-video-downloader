...
- **TikTok rehydration (Rank 2, Analyze-only — deployed, healthy):** stderr matching **`Unable to extract universal data for rehydration`** classifies as **`tiktok_rehydration`** (not `unknown`). Analyze retries metadata extraction **at most once immediately** when host is TikTok and the first attempt is that class (max **2** attempts total). Non-eligible failures are **not** retried (unsupported/photo/auth/private/403/429/format/generic/other platforms). Worker/download retry behavior unchanged. Client response codes unchanged (`ANALYZE_FAILED` when still failing). Safe attempt logs: `attempt`, `retryEligible`, `retryResult` on `[Perf][Analyze]` plus `analyzeTikTokRetry` structured fields (no URL/stderr/cookies). Production smoke passed; **organic rehydration sample is still too small to quantify retry recovery rate** — further observation required before declaring TikTok Analyze reliability resolved. Regression: `cd backend && npm run diag:analyze-tiktok-retry`.
...
**TikTok Analyze rehydration retry checkpoint (Rank 2, deployed 2026-08-09 ~02:51 UTC — healthy):** Narrow **`tiktok_rehydration`** classification + **Analyze-only** immediate retry (**max one retry** / **2** metadata attempts). Non-eligible failures are not retried. Image **`sha256:9df6eca40b…`** (was Rank 1 **`8b2e11d3…`**). Dockerfile / Rank 1 curl_cffi / worker retries / yt-dlp flags unchanged. Production smoke passed (health 200, Rank 1 still active, first-attempt success correctly skips retry, no stop conditions). **Organic sample is currently too small to quantify recovery rate** — further observation required before declaring TikTok Analyze reliability resolved. **Do not document as fully fixed.**
...
2. **TikTok Analyze reliability (open — Rank 2 deployed)** — Narrow Analyze-only rehydration retry is live and healthy; **continue observing organic rehydration recovery rate** before calling reliability resolved. Early `/photo/` rejection remains a separate follow-up.
