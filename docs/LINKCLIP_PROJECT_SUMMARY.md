...
| `lib/features/edit/` | Quick Edit UI (horizontal **scrollable** tool strip: Trim → Speed → Format → Captions → Audio → Quality — **RTL-aware overflow arrows + edge fades** when tabs clip), **Captions V1.5–V2.3** panel (**Auto captions** + **V2.3** horizontal **Preset** chips: Minimal / Social / Bold Yellow / Dark Box / Top Clean + inferred **Custom**; manual size / position / color / style + **fine-tune offsets**; default caption controls align with **Minimal**; **`captions`** JSON is still explicit **`style` / fontSize / position / color / `offsetX` / `offsetY`** — preset key not sent), **approximate captions overlay on `EditVideoPreview`** when Captions tab + Auto on (upright, `IgnorePointer`); expired-source sheet, trim/speed/format/audio/quality controls |
