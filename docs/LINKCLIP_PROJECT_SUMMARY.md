...
| `lib/features/edit/` | Quick Edit UI (horizontal **scrollable** tool strip: Trim → Speed → Format → Captions → Audio → Quality — **RTL-aware overflow arrows + edge fades** when tabs clip), **Captions V1.5** panel (**Auto captions** + **extra small**–large size / position / color / style + **approximate preview**), expired-source sheet, trim/speed/format/audio/quality controls |
