...
| `lib/features/edit/` | Quick Edit UI (horizontal **scrollable** tool strip: Trim → Speed → Format → Captions → Audio → Quality — **RTL-aware overflow arrows + edge fades** when tabs clip), **Captions V1.5–V3.2** panel (**V3** three-card layout + **V3.1** draft editor screen; **V3.2 Style Pack**: Preset / Text size (**extra_small→xx_large**) / **Font** / Position in **Look**; advanced **Accent** (4 colors) + **Style** (8 recipes) + fine-tune offsets under **More styling options**; **`captions`** JSON is explicit **`style` / fontSize / fontFamily / position / color / `offsetX` / `offsetY`** — preset key not sent), **approximate captions overlay on `EditVideoPreview`** when Captions tab + Auto on (upright, `IgnorePointer`, **google_fonts** for preview); expired-source sheet, trim/speed/format/audio/quality controls |
