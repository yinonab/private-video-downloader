...
**Caption typography checkpoint (2026-08-04):** Shared export `baselineY` for word-highlight canvas; box highlight plate optically centered from font/ink metrics around that baseline (text draw Y unchanged). Manual QA passed — no Hebrew descender clipping, no baseline/bobbing regression. Remaining micro variance between Hebrew glyphs accepted as natural typography. **Phase A line-break SoT:** backend ASS + highlight timing + highlight plate consume one shared greedy `breakCaptionLines` (`captionLineBreak.ts`); Preview still uses Flutter softWrap (Phase B pending). Details: `docs/CAPTION_TYPOGRAPHY_TRACKER.md`.
