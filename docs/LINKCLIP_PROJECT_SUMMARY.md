...
| `lib/features/edit/` | Quick Edit UI (horizontal **scrollable** tool strip: Trim → Speed → Format → Captions → Audio → Quality — **RTL-aware overflow arrows + edge fades** when tabs clip), **Captions V1.5–V3.4D** panel (**V3** three-card layout + **V3.1** draft editor + **V3.4D** **`CaptionLookEditorScreen`** opened via **Customize look** / **התאמת מראה** with tabs Presets / Text / Highlight / Position; **V3.4G** **CaptionPreviewCard** cinematic stage (~120px, generous safe insets, animated motion); **Position** tab **controller-style D-pad** (four raised directional buttons, tap + long-press repeat, HUD readout) + pill Top/Bottom; **Presets** 2-column compact grid when width ≥ 340px; compact **Look** summary on main panel; full styling (fonts, swatches, box shapes, presets incl. Pink Pop / Yellow Viral / Clean Focus) in look editor only; **`captions`** JSON is explicit **`style` / fontSize / fontFamily / position / color / wordHighlight / `offsetX` / `offsetY`** — preset key not sent), **approximate captions overlay** with static highlighted sample word when word highlight is on; draft segments preserve optional **`words[]`** for **`mode=segments`** burn (no per-word editor); expired-source sheet, trim/speed/format/audio/quality controls |
