...
**LinkClip** is an Android-first MVP that lets users **analyze** a shared or pasted video URL, **download** media via a backend worker, **edit** videos from links or from the device (**Downloads** / **Edits** areas on Home), then **open**, **share**, or **save** files locally. **Quick Edit** runs **on the server** (ffmpeg): trim, optional **rotate** (clockwise **90° / 180° / 270°** pixel rotation — **0° omits the op**; from the **Format** panel, not a separate tab), **format** (Fill / Fit+blur; applied **after** rotation), **constant playback speed** (0.5×–2× presets; **1×** omits the op), optional **captions** (Android **Captions / כתוביות** tab — **Auto captions** + **V1.5 styling** + **V2.3 presets** (scrollable chips → same explicit `style` / fontSize / position / color / offsets in API; **no** preset key) + **V2.2 fine positioning** (**`offsetX`** / **`offsetY`**, arrows + reset in panel; **`EditCaptionsPreviewOverlay`** on real **`EditVideoPreview`** when Captions tab is active; backend ASS `\an` + **`\pos`** burn-in honors the same offsets, clamped) + **V1.5.1 rendering fixes**: **`extra_small`–`large`** size, top/bottom position, white/yellow, clean/bold/dark-box presets; **`captions`** op only when enabled; Whisper + burned-in MP4 — **requires `OPENAI_API_KEY`** on backend), mute, compress. **No translation, manual caption editor, free font/color pickers, finger-drag positioning, or client-side OpenAI.** **No** flip/mirror, free-angle rotation, or client-side ffmpeg for Quick Edit.
...
- `POST /edits` — body includes **exactly one** of **`sourceDownloadJobId`** (completed download job, existing Quick Edit) **or** **`sourceUploadId`** (`UploadedMedia` id from Phase A upload API), plus **`operations`** (trim, **`rotate`** with **`degrees`**: **90 / 180 / 270** — **0°** must not appear; invalid values → **`UNSUPPORTED_ROTATION`**; **`format`** with `aspectRatio` + **`mode`**: **`fill`** or **`fit_blur`** — omitted defaults to **`fill`**; legacy **`crop`** still accepted and implies fill; **`speed`** with fixed factors **0.5 / 1.25 / 1.5 / 2** — factor **1** must not appear; **`UNSUPPORTED_SPEED_FACTOR`**); invalid format **`mode`** → **`UNSUPPORTED_FORMAT_MODE`**; **`captions`** **V1.5 / V2.2 / V2.4**: **`language`**: **`auto`**, **`burnIn`**: **`true`**; **`mode`**: **`auto`** (Whisper transcription in worker after trimmed/sped timeline mux) **or** **`segments`** (client‑supplied cues after **`POST /edits/captions/draft`** review — **requires** non‑empty **`segments`** array of **`{ startSec, endSec, text }`** (each cue **≥0.3s** duration — **V2.4B** mobile clamp; invalid → **`INVALID_CAPTION_SEGMENTS`**); server normalizes overlaps/empties and **does not** call Whisper for burn‑in); **`OPENAI_API_KEY`** is required only when a job uses **`captions.mode auto`** **or** the user requests a **draft**; **`style`**: **`default`** \| **`clean`** \| **`bold`** \| **`dark_box`** (resolved **`default`** → **`clean`**); optional **`fontSize`** / **`position`** / **`color`** with defaults **medium** / **bottom** / **white**; optional **`offsetX`** (−240…240) / **`offsetY`** (−180…180), default **0** (invalid → **`UNSUPPORTED_CAPTIONS_POSITION_OFFSET`**, surfaced like position errors); granular **`UNSUPPORTED_CAPTIONS_*`** / **`INVALID_CAPTION_SEGMENTS`** on invalid values; ASS burn-in applies **preset styling**, per-event **`\pos`** anchors (**`\an2`** bottom / **`\an8`** top), **2-line wrapping**, **chunked timed events** for long cues (`assSubtitles.service.ts`); **missing API key when `captions.mode auto`** → **`CAPTIONS_TRANSCRIPTION_UNAVAILABLE`** (HTTP **503**); **`CAPTIONS_GENERATION_FAILED`** on Whisper failures in the worker; **`CAPTIONS_DRAFT_UNAVAILABLE`** when draft transcription fails; mute, compress — validated in `edit.schemas.ts`). Codes: **`EDIT_SOURCE_REQUIRED`**, **`EDIT_MULTIPLE_SOURCES`**, **`EDIT_UPLOAD_NOT_FOUND`**, **`EDIT_UPLOAD_NOT_READY`**, **`EDIT_SOURCE_FILE_MISSING`** where applicable.
- `POST /edits/captions/draft` — **Captions Draft V2.4A**: same XOR **`source*`** fields as **`POST /edits`**, **`operations`** accept **only** **`trim`** + **`speed`** so draft timestamps align with edited output timeline; synchronous handler builds the same preprocessor mux ffmpeg uses before Whisper (**no subtitle burn yet**); returns **`segments`** with stable **`id`**, Whisper **`start/end`**, **`text`**, **`durationSec`**, **`language: auto`**. Errors: **`CAPTIONS_DRAFT_UNAVAILABLE`**. Logs avoid transcript payload (**counts / duration / model** only).
...
| `lib/features/edit/` | Quick Edit UI (horizontal **scrollable** tool strip: Trim → Speed → Format → Captions → Audio → Quality — **RTL-aware overflow arrows + edge fades** when tabs clip), **Captions V1.5–V2.3** panel (**Auto captions** + **V2.3** horizontal **Preset** chips (Minimal … Top Clean only) + **Manual** subtitle pill when combos deviate from a built‑in preset; manual size / position / color / style + **fine-tune offsets**; default caption controls align with **Minimal**; **`captions`** JSON is still explicit **`style` / fontSize / position / color / `offsetX` / `offsetY`** — preset key not sent), **approximate captions overlay on `EditVideoPreview`** when Captions tab + Auto on (upright, `IgnorePointer`); expired-source sheet, trim/speed/format/audio/quality controls |
...
- **`EditVideoScreen`** — **scrollable** tool strip (**overflow arrows + soft edge fades**, RTL-aware) + single visible panel: **Trim**, **Speed**, **Format** (shape + Fill/Fit-blur + **rotation** — not a separate tab), **Captions** (**Captions UX V3 / V3.1**: three cards — **Add captions** → **Caption text** (compact **Draft ready** summary + **Edit captions** opens **`CaptionDraftEditorScreen`**) → **Look**; full segment list moved off the main panel; advanced Color/Style/Fine-tune collapsed under **More styling options**; stronger chip selected states; polished single-line **`EditCaptionsPreviewOverlay`**; **Auto captions** + **V2.4A/B draft**: **Generate draft** / **Regenerate draft** (confirmation) on **`trim`**/**`speed`** timeline via **`POST /edits/captions/draft`**, **segment text + timing edits** via reused keyboard-aware bottom sheet in the full-screen editor; Save/Cancel/Clear; empty text allowed locally; **no** waveform/timeline drag/**translation** yet), timing/source invalidation helpers + **V2.3 presets** + manual sizing / positioning / offsets; default control values match **Minimal** preset), **Audio** (mute), **Quality** (compress preset); compose → **`POST /edits`** (**`captions.mode auto`** without draft vs **`segments`** when draft loaded — burns edited **text + timing** without OpenAI) → poll → download. **No backend/API changes in V3/V3.1.**
