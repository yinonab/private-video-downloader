...
| Acquisition | **yt-dlp** with **`[default,curl-cffi]`** (impersonation available; **TikTok Analyze/download additionally pass a desktop `--user-agent`** so curl_cffi impersonation is not used for TikTok webpage fetch — see TikTok UA note below), **yt-dlp-ejs**, **ffmpeg** / **ffprobe** |
...
| YouTube / JS challenges | **Node 22+** + `yt-dlp-ejs`; `--no-js-runtimes --js-runtimes node` (`YTDLP_JS_RUNTIME_ARGS`). Docker base **`node:22-bookworm`** (Node 20 is unsupported by current yt-dlp EJS → n-challenge fails → only storyboard/image formats → Analyze `format_unavailable`) |
...
- **TikTok yt-dlp User-Agent (deployed 2026-08-11 ~12:02 UTC, KEEP_AND_COMMIT):** **Reproducible TikTok Site Maintenance failure fixed for first-class short-link input; upstream TikTok extraction remains intermittently transient and continues to be protected by Rank 4 retries.** Root cause: TikTok’s yt-dlp path uses `impersonate=True` (curl_cffi); on production egress that fingerprint received a ~537-byte **Site Maintenance** HTML page → `tiktok_webpage_unexpected`, while plain HTTP clients (urllib/curl/Node) from the **same IP** got a normal page with `__UNIVERSAL_DATA_FOR_REHYDRATION__`. Fix: `withYtDlpTikTokUserAgentArgs` transparently prepends a desktop Chrome `--user-agent` for **`hostnameIsTikTok` hosts only** in `fetchMetadataJson` (Analyze) and `buildDownloadArgs` (Worker), which disables the problematic impersonation behavior (upstream yt-dlp). Optional override: **`YTDLP_TIKTOK_USER_AGENT`**. **`vt.tiktok.com` short links remain first-class** — users do not manually canonicalize. YouTube / Instagram / Facebook / X do **not** receive the TikTok UA. **No proxy, paid egress, alternate VPS, or custom TikTok scraper.** Rank 4 unchanged (max **3** transient attempts; BullMQ **`attempts: 1`**; no attempt 4). Production image **`sha256:c553cefeb654…`** (pre-UA rollback `linkclip-backend:pre-tiktok-ua-*` / prior Rank 4 **`sha256:faf17c45d180…`**). Guided QA (approved short URL): Analyze HTTP **200** (attempt1 `tiktok_rehydration` → attempt2 success) → Worker primary attempt **done**, **one** FileAsset (~5.1 MB MP4), ffprobe ~95.2s HEVC+AAC, no terminal EventLog/Slack on success; Site Maintenance absent on app path; YouTube Analyze regression OK (no UA leak). Limitation: a separate canonical retest still exhausted Rank 4 (`tiktok_rehydration` → `tiktok_webpage_unexpected` ×2 → terminal failure) — **not 100% solved**. Regression: `npm run diag:tiktok-user-agent-args`.
- **TikTok transient extraction (Rank 4 — deployed 2026-08-10 ~13:31 UTC, KEEP after guided production smoke):** Adds **`tiktok_webpage_unexpected`** (`Unexpected response from webpage request`, TikTok-host-gated only). Approved transient family: **`tiktok_rehydration` | `tiktok_webpage_unexpected`**. Shared gate `isTikTokTransientExtractionRetryEligible` allows up to **3** primary yt-dlp attempts (Analyze + download worker) only while the **current** failure stays in that family; hard/non-retryable failures stop immediately. Immediate retries (no artificial delay). **BullMQ `attempts` remains 1.** Format-unavailable fallbacks run **after** the primary transient loop. Terminal Slack / EventLog only after retries are exhausted or a non-retryable terminal failure — **not** after intermediate eligible attempts. Precedence: primary attempt(s) → transient retry if eligible → then existing format fallback if applicable. Safe logs: `analyzeTikTokRetry` / `downloadTikTokRetry` (`attempt`, `maxAttempts`, `transientFamily`, `retryEligible`, `retryStarted`, `retryResult`). Image **`sha256:faf17c45d180…`** (rollback **`linkclip-backend:rank3-20260809`** = `sha256:df2f9b8172…`). Rank 4 **improves recovery** but **does not eliminate** TikTok upstream/extraction failures — **do not mark TikTok reliability as fully solved**. Regressions: `npm run diag:ytdlp-classify`, `npm run diag:analyze-tiktok-retry`, `npm run diag:download-tiktok-retry`.
...
- **Download / Analyze performance:** Safe stage logs (`[Perf][Download]`, `[Perf][Analyze]`, `[Perf][MobileDownload]`). **TikTok-ready:** yt-dlp format selector prefers progressive AVC/H.264 (+ AAC when tagged), then merge arms, then the former `best` fallback (`YT_DLP_FORMAT_TIKTOK_READY`); post-download ffprobe/normalize still always runs. Normalize and **`GET /downloads/:id/file` streaming** logs use path-free fields (sizes/mime/exists — no storage keys or absolute paths). **`best` unchanged.** See `docs/LINKCLIP_DOWNLOAD_PERFORMANCE.md`.
...
**TikTok Analyze rehydration retry checkpoint (Rank 2, deployed 2026-08-09 ~02:51 UTC — healthy):** Narrow **`tiktok_rehydration`** classification + **Analyze-only** immediate retry (**max one retry** / **2** metadata attempts). Superseded by Rank 4 (max **3**, expanded family) deployed 2026-08-10.

**TikTok download-worker rehydration retry checkpoint (Rank 3, deployed 2026-08-09 ~14:13 UTC — healthy):** Worker **one immediate retry** for `tiktok_rehydration` on primary yt-dlp (max **2**). **BullMQ `attempts: 1` unchanged.** Image **`sha256:df2f9b8172…`** retained as rollback tag `rank3-20260809`. Superseded by Rank 4 deployed 2026-08-10.

**TikTok same-IP User-Agent checkpoint (deployed 2026-08-11 ~12:02 UTC — KEEP_AND_COMMIT):** TikTok-only desktop Chrome `--user-agent` on Analyze + Worker. **Reproducible TikTok Site Maintenance failure fixed for first-class short-link input; upstream TikTok extraction remains intermittently transient and continues to be protected by Rank 4 retries.** Image **`sha256:c553cefeb654…`**. Guided QA: `vt.tiktok.com` short link Analyze 200 (rehydration→attempt2) → Worker attempt1 **done**, one FileAsset (~5.1 MB), ffprobe OK; YouTube Analyze OK; no TikTok UA leak. Canonical retest still exhausted Rank 4 once — observation mode, not a new reliability phase.

**TikTok transient extraction retry checkpoint (Rank 4, deployed 2026-08-10 ~13:31 UTC — KEEP):** Transient family **`tiktok_rehydration` | `tiktok_webpage_unexpected`**; max **3** primary yt-dlp attempts; current failure must remain in family for each next attempt; hard failures stop immediately; BullMQ **`attempts: 1`**; terminal Slack/EventLog only after exhaustion / non-retryable terminal. Image **`sha256:faf17c45d180…`**; Rank 3 rollback **`linkclip-backend:rank3-20260809`**. Guided smoke (3 known prior TikTok videos): **Sample 1** Analyze rehydration→attempt2 success; Worker rehydration→attempt2 success; **done**, one FileAsset, no terminal alert. **Sample 2** Analyze attempt1 success; Worker rehydration→webpage_unexpected→webpage_unexpected→**terminal failed**; **exactly one** Slack alert and **one** terminal EventLog **after attempt 3** (none after attempts 1–2); zero FileAssets; no attempt 4 — **expected correct terminal semantics, not a regression**. **Sample 3** Analyze attempt1 success; Worker rehydration→attempt2 success; **done**, one FileAsset. Patterns **A**, **B**, **D** verified; pattern **C** (attempt-3 recovery) not observed yet. No attempt >3, no duplicate FileAssets, no premature terminal Slack/EventLog, health **200**, RestartCount **0**. Effectiveness remains under observation — **not fully solved**.
...
2. **TikTok reliability (observation)** — Site Maintenance wall fixed via TikTok-only UA (`sha256:c553cefeb654…`); Rank 4 remains safety net (max 3). Do **not** mark TikTok 100% solved. Early `/photo/` rejection remains a separate follow-up.
