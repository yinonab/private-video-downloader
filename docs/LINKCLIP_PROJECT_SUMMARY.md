...
- `POST /edits` — body includes **exactly one** of **`sourceDownloadJobId`** (completed download job, existing Quick Edit) **or** **`sourceUploadId`** (`UploadedMedia` id from Phase A upload API), plus **`operations`** (trim, **`rotate`** with **`degrees`**: **90 / 180 / 270** — **0°** must not appear; invalid values → **`UNSUPPORTED_ROTATION`**; **`format`** with `aspectRatio` + **`mode`**: **`fill`** or **`fit_blur`** — omitted defaults to **`fill`**; legacy **`crop`** still accepted and implies fill; **`speed`** with fixed factors **0.5 / 1.25 / 1.5 / 2** — factor **1** must not appear; **`UNSUPPORTED_SPEED_FACTOR`**); invalid format **`mode`** → **`UNSUPPORTED_FORMAT_MODE`**; **`captions`** **V1** — only `{ "type":"captions","mode":"auto","language":"auto","burnIn":true,"style":"default" }` accepted; other values → **`UNSUPPORTED_CAPTIONS_*`**; **burned-in** pipeline uses OpenAI transcription + temporary ASS + ffmpeg `subtitles` filter (see `transcription.service.ts`, `assSubtitles.service.ts`, two-pass edit worker when captions present); **missing `OPENAI_API_KEY`** when captions requested → **`CAPTIONS_TRANSCRIPTION_UNAVAILABLE`** (HTTP **503**); transcription/API failures → **`CAPTIONS_GENERATION_FAILED`**; mute, compress — validated in `edit.schemas.ts`). Codes: **`EDIT_SOURCE_REQUIRED`**, **`EDIT_MULTIPLE_SOURCES`**, **`EDIT_UPLOAD_NOT_FOUND`**, **`EDIT_UPLOAD_NOT_READY`**, **`EDIT_SOURCE_FILE_MISSING`** where applicable.
...
- **Operations:** trim (time range), optional **`rotate`** (**90 / 180 / 270** clockwise, pixel **transpose**/**flip** — not metadata-only; **0°** = omit op), **`format`** (preset aspect ratios; **`fill`** = crop-to-cover; **`fit_blur`** = fit-in-frame + blurred background; legacy **`crop`** = fill), optional **`captions` V1** (auto transcription + burned-in ASS subtitles in **original spoken language**; **requires `OPENAI_API_KEY`** and **`OPENAI_TRANSCRIPTION_MODEL`** defaulting to **`whisper-1`**; **two-pass FFmpeg** when captions requested: trimmed/sped/normalized timeline MP4 → **Whisper WAV** transcript → ASS → burn + final mute/compress; **silent / empty segments**: export **without overlays**, no fatal), **speed** (**0.5× / 1.25× / 1.5× / 2×**; normal **1×** omits the op entirely), mute, compress (tiered encoding — see schemas/service). FFmpeg conceptual order: trim → **rotate** → spatial **format**, **`setpts`/`atempo`** on A/V (**before Whisper**); **mute** affects **final mux audio only**, not the transcript source when captions are requested.
