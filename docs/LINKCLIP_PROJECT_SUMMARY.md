...
- **Operations:** trim (time range), optional **`rotate`** (**90 / 180 / 270** clockwise, pixel **transpose**/**flip** — not metadata-only; **0°** = omit op), **`format`** (preset aspect ratios; **`fill`** = crop-to-cover; **`fit_blur`** = fit-in-frame + blurred background; legacy **`crop`** = fill), optional **`captions` V1.5 (+ V1.5.1 rendering)** (auto transcription + styled/burned-in ASS in **original language**; **requires `OPENAI_API_KEY`** + **`OPENAI_TRANSCRIPTION_MODEL`** default **`whisper-1`**; **fontsize** presets **extra_small→16, small→20, medium→24, large→30**; **≤2 ASS lines/event** via manual wrap + **time-chunking** overflow; **`WrapStyle` 2** + `\N`-safe escaping; larger vertical **MarginV**; **two-pass** worker: timeline MP4 → **Whisper** → ASS → burn + final mute/compress; **empty segments**: no burn, no fatal), **speed** (**0.5× / 1.25× / 1.5× / 2×**; **1×** omits op), mute, compress (tiered encoding — see schemas/service). FFmpeg conceptual order: trim → **rotate** → spatial **format**, **`setpts`/`atempo`** on A/V (**before Whisper**); **mute** affects **final mux audio only**, not the transcript source when captions are requested.
