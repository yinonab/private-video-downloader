...
- **Edit screen:** Premium minimal layout — large preview, **horizontal scrollable tool strip** (Trim → **Speed** → Format → **Captions** → Audio → Quality) with **subtle overflow affordances** (directional arrows + soft gutter fades when chips extend past the viewport — **RTL-safe**), **one tool panel at a time** (no 2×2 grid, no `TabBarView` swipe), soft panel surfaces, **Create edit** primary CTA (`edit_video_screen.dart`, trim/format/speed/captions/compression widgets, l10n). **Format** panel: video shape, Fill vs Keep-all, and **Rotation (0° default; 90° / 180° / 270° clockwise pixel rotation before fill/fit)**. **Speed** is **constant for the entire output only** (`setpts`/conditional `atempo` server-side — no ramps, curves, beats, timeline markers). **Captions V1.5 / V3.2:** **Auto captions** (**off** by default); when **on**, **extra small → XXL** size / **font** / position / accent color / style chips + **approximate preview overlay** (not FFmpeg parity); **V3.6 preview parity:** in-editor + **fullscreen** video preview uses **contain** letterboxing (full frame, no cover crop), caption overlay anchored to the **visible video rect**, **edit-timeline** playback (post trim + speed) for draft cue/word highlight, sample text only on the isolated **Look** stage card — not over real video when a draft exists; fullscreen unlocks **portrait + landscape** while open then restores app portrait lock. Defaults **clean_pro / medium / default font / bottom / white**. Draft text/timing editor + **V3.2** presets (**Creator Highlight**, **News Headline**, …). No translation/word‑karaoke/export/free color pickers.
