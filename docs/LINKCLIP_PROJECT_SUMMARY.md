...
- **Download / Analyze performance instrumentation:** Safe wall-clock stage logs for job create, queue wait, yt-dlp, ffmpeg (or skipped), backend totals (`[Perf][Download]`), Analyze sub-stages (`[Perf][Analyze]`: platform detect, yt-dlp `--dump-json`, qualities, Link upsert, total), and mobile create/poll/cache/save/share/open/analyze_http (`[Perf][MobileDownload]`, debug builds). Share prep vs native sheet return split. See `docs/LINKCLIP_DOWNLOAD_PERFORMANCE.md` — no yt-dlp/ffmpeg/analyze-cache optimizations yet.
