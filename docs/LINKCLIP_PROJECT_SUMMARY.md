...
- **TikTok rehydration (Rank 2 Analyze + Rank 3 worker — both deployed and healthy):** stderr matching **`Unable to extract universal data for rehydration`** classifies as **`tiktok_rehydration`**. **Analyze** retries metadata extraction **at most once immediately** when host is TikTok (max **2** attempts). **Download worker** performs **one immediate retry only** when `classification === "tiktok_rehydration"` on the **primary** yt-dlp download invocation (max **2** primary attempts). **BullMQ `attempts` remains 1.** Rank 1 curl_cffi / Rank 2 Analyze path unchanged. Format-unavailable fallbacks unchanged. Terminal Slack / EventLog fire only after the worker’s final failed extraction (not after a rehydration attempt that will be retried). Safe logs: Analyze `analyzeTikTokRetry`; worker `downloadTikTokRetry` (`attempt`, `retryEligible`, `retryResult`, `context=download_worker`). Production smoke passed. **Organic production sample is still too small to quantify recovery rate** — TikTok reliability is **improved but still under observation**; **do not describe as fully solved**. Regressions: `npm run diag:analyze-tiktok-retry`, `npm run diag:download-tiktok-retry`.
...
**TikTok Analyze rehydration retry checkpoint (Rank 2, deployed 2026-08-09 ~02:51 UTC — healthy):** Narrow **`tiktok_rehydration`** classification + **Analyze-only** immediate retry (**max one retry** / **2** metadata attempts). Non-eligible failures are not retried. Production smoke passed. **Organic Analyze rehydration sample remains too small to quantify recovery rate.**

**TikTok download-worker rehydration retry checkpoint (Rank 3, deployed 2026-08-09 ~14:13 UTC — healthy):** Worker performs **one immediate retry only** for `classification == "tiktok_rehydration"` on the primary yt-dlp download path (max **2** primary attempts). **BullMQ `attempts: 1` unchanged.** Rank 1 + Rank 2 unchanged. Image **`sha256:df2f9b8172…`**. Production smoke passed (health 200, diags green, no stop conditions). **Organic production sample is still too small to quantify recovery rate** — TikTok reliability is improved but **still under observation**; **do not describe as fully solved.** Next step: production monitoring only until enough real rehydration events exist to measure recovery rate.
...
2. **TikTok reliability (Rank 2 + Rank 3 deployed — observation mode)** — Analyze and worker rehydration retries are live and healthy; **continue monitoring organic recovery rate** before calling reliability resolved. Early `/photo/` rejection remains a separate follow-up.
