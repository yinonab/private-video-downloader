...
- **`EditVideoScreen`** — **scrollable** tool strip (**overflow arrows + soft edge fades**, RTL-aware) + single visible panel: **Trim**, **Speed**, **Format** (shape + Fill/Fit-blur + **rotation** — not a separate tab), **Captions** (**Auto captions** + **V2.4A/B draft**: **Generate draft** / **Regenerate draft** (confirmation before replacing cue edits) on **`trim`**/**`speed`** timeline via **`POST /edits/captions/draft`**, **segment text + timing edits in a keyboard-aware bottom sheet** (read-only preview rows; ±**0.1s** start/end nudges, per-segment **Reset timing**, **Adjusted** badge when timing differs from Whisper draft; Save/Cancel/Clear; empty text allowed locally; **no** waveform/timeline drag/**translation** yet), timing/source invalidation helpers + **V2.3 presets** + manual sizing / positioning / offsets; default control values match **Minimal** preset; **`EditCaptionsPreviewOverlay`** remains style/position preview only), **Audio** (mute), **Quality** (compress preset); compose → **`POST /edits`** (**`captions.mode auto`** without draft vs **`segments`** when draft loaded — burns edited **text + timing** without OpenAI) → poll → download.
