...
| `lib/features/edit/` | Quick Edit UI (horizontal tool strip: Trim → Speed → Format → Captions → Audio → Quality), **Captions V1.5** panel (**Auto captions** + size / position / color / style + approximate preview chip), expired-source sheet, trim/speed/format/audio/quality controls |
...
- **Operations:** trim (time range), optional **`rotate`** (**90 / 180 / 270** clockwise, pixel **transpose**/**flip** — not metadata-only; **0°** = omit op), **`format`** (preset aspect ratios; **`fill`** = crop-to-cover; **`fit_blur`** = fit-in-frame + blurred background; legacy **`crop`** = fill), optional **`captions` V1.5** (auto transcription + styled/burned-in ASS in **original language**; **requires `OPENAI_API_KEY`** + **`OPENAI_TRANSCRIPTION_MODEL`** default **`whisper-1`**; **wrap + chunk** long cues; **two-pass** worker: timeline MP4 → **Whisper** → ASS → burn + final mute/compress; **empty segments**: no burn, no fatal), **speed** (**0.5× / 1.25× / 1.5× / 2×**; **1×** omits op), mute, compress (tiered encoding — see schemas/service). FFmpeg conceptual order: trim → **rotate** → spatial **format**, **`setpts`/`atempo`** on A/V (**before Whisper**); **mute** affects **final mux audio only**, not the transcript source when captions are requested.
...
- **`EditVideoScreen`** — tool strip + single visible panel: **Trim**, **Speed**, **Format** (shape + Fill/Fit-blur + **rotation** — not a separate tab), **Captions** (**Auto captions** + **V1.5** appearance controls when enabled — defaults **clean / medium / bottom / white**), **Audio** (mute), **Quality** (compress preset); compose → `POST /edits` → poll → download.
