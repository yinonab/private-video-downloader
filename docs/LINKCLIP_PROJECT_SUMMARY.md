...
- **Quick Edit processing:** Calm progress + optional framed hero animation; rings stay in the muted blue/slate family (`edit_processing_animation.dart`). **Foreground operation wake lock:** screen stays awake during active link analyze, server download polling, local save-to-device, caption draft generation (visible wait), and edit/export (`wakelock_plus` via refcounted `operation_wakelock.dart`); released on success/failure/cancel/back/dispose — not held on idle or post-success result screens. **Export progress UX:** staged localized headlines + capped progress bar (`edit_progress_display.dart` — backend `stage`/`progressPercent` when present, time-based estimate otherwise; caption-aware burn messages).
- **Edit screen:** Premium minimal layout — large preview, **horizontal scrollable tool strip** (Trim → **Speed** → Format → **Captions** → Audio → Quality) with **subtle overflow affordances** (directional arrows + soft gutter fades when chips extend past the viewport — **RTL-safe**), **one tool panel at a time** (no 2×2 grid, no `TabBarView` swipe), soft panel surfaces, **Create edit** primary CTA (`edit_video_screen.dart`, trim/format/speed/captions/compression widgets, l10n). **Unified edit preview foundation** (`edit_preview_state.dart`, `docs/EDIT_PREVIEW_CAPABILITY_MATRIX.md`): passive `EditPreviewState` drives preview **speed**, **mute**, caption overlay, crop guide — export via `buildQuickEditOperations`. **Caption preview consistency**: no-draft explanation + CTA in Captions tab (no sample on video); live draft working-copy (`onDraftChanged` / segment `onLiveChanged`); draft editor in draggable sheet (preview stays visible); output cards use `resolveMediaOutputPreview` / `MediaOutputPreviewThumbnail` (final MP4 frame over source thumb). **Format** panel: video shape, Fill vs Keep-all, and **Rotation (0° default; 90° / 180° / 270° clockwise pixel rotation before fill/fit)**. **Speed** is **constant for the entire output only** (`setpts`/conditional `atempo` server-side — no ramps, curves, beats, timeline markers). **Captions V1.5 / V3.2:** **Auto captions** (**off** by default); when **on**, **extra small → XXL** size / **font** / position / accent color / style chips + **approximate preview overlay** on the **Captions** tab (selected style + **real draft cue** at `_playbackSec` when draft loaded; sample text only on **`CaptionPreviewCard`** in Look Editor; in-editor video uses **contain** letterboxing at 0°/180°); defaults **clean_pro / medium / default font / bottom / white**. Draft text/timing editor + **V3.2** presets (**Creator Highlight**, **News Headline**, …). No translation/word‑karaoke/export/free color pickers.
