...
- **Download performance instrumentation:** Safe wall-clock stage logs for analyze, job create, queue wait, yt-dlp, ffmpeg (or skipped), backend totals (`[Perf][Download]`), and mobile create/poll/cache/save/share/open (`[Perf][MobileDownload]`, debug builds). No yt-dlp/ffmpeg/behavior changes. See `docs/LINKCLIP_DOWNLOAD_PERFORMANCE.md`.
